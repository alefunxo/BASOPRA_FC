...
def load_param(combinations):
    ...
    fields_el=['index',id_dwell,'E_PV','Price_flat','Price_DT','Price_flat_mod','Price_DT_mod','Export_price','FC_price_up',
               'FC_price_down','FC_activation_up','FC_activation_down']
...
######################################################
    ...
    data_input=pd.concat([df.loc[:,'E_demand'],
                         ...
                         df.loc[:,'Export_price'],
                         df.loc[:,'FC_price_up'],
                         df.loc[:,'FC_price_down'],
                         df.loc[:,'FC_activation_up'],
                         df.loc[:,'FC_activation_down']],axis=1)
    print('bf test')
    if testing:
